...
logger = logging.getLogger("OpenScienceOAI")
...
class OpenScienceOAIClient:
    """A client library specifically configured for the LIBNAS Open Science repository:
    http://open-science.nbuv.gov.ua/oai
    """
    
    DEFAULT_BASE_URL = "http://open-science.nbuv.gov.ua/oai"
    ...
    def list_sets(self) -> Generator[Dict[str, str], None, None]:
        """Yields all sets in the repository.
        
        Note: Since open-science.nbuv.gov.ua does not support sets, this will
        gracefully log a warning and return an empty generator instead of crashing.
        """
        try:
            root = self._request('ListSets')
            list_sets_el = root.find('oai:ListSets', self.ns)
            if list_sets_el is not None:
            ...
            if e.code == 'noSetHierarchy':
                logger.warning("This repository does not support sets (noSetHierarchy). Yielding empty results.")
            else:
                raise
    ...
    def list_records(self, metadata_prefix: str = "oai_dc", set_spec: Optional[str] = None) -> Generator[Dict[str, Any], None, None]:
        ...
        while True:
            try:
                ...
                # If set is not supported or no records match
                if e.code == 'noRecordsMatch':
                    logger.info("No records match the query.")
                    break
                raise
